...
function buffAnchor:OnUpdate(dt)
    ...
    elseif nScrZ_Tar > 0 then
        ...
        for i = 1, UBuffCount do
            ...
            if self_buffs[buff["name"]] ~= nil then
                currentBuffs[buff["name"]] = true
                local iconPath = self_buffs[buff["name"]]
        ...
        for i = 1, UDebuffCount do
            ...
            if self_debuffs[debuff["name"]] ~= nil then
                currentBuffs[debuff["name"]] = true
                local iconPath = self_debuffs[debuff["name"]]
        ...
        if self_buffDebugMessages and (buffAllString ~= lastbuffString) then
            lastbuffString = buffAllString
        end
        if self_debuffDebugMessages and (debuffAllString ~= lastdebuffString) then
